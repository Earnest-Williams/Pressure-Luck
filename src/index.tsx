...
/**
 * Push-Your-Luck — Faces & Dice (React single-file)
 *
 * Major features implemented:
 * - Per-die stats (sides & winning faces); click a die to select for upgrades.
 * - Heat system: base heat starts at 1.0 and adds +0.1 per side above 2 across all dice (d3 = +0.1, d4 = +0.2, ...).
 *   Streak adds +0.05 per success, but ONLY whole-number heat levels (x2, x3, ...) affect payout.
 * - Base payout per winning die scales with dice count: 1.0 + 0.1×(dice−1).
 * - Add-a-Die costs: 20, then doubles per extra die; any NON-die purchase increases
 *   future Add-a-Die prices by +25% multiplicatively (independent of doubling curve).
 * - Auto Roller (buy/upgrade): reduces roll interval by 25% per level; speed scales indefinitely (min 10ms).
 * - Auto Banker (buy, set target): banks when pot ≥ target.
 * - Destroy a die: halves ALL current buy prices (global scale ×0.5) and adds +0.1%/tick interest per side
 *   of the destroyed die (compounds once per second).
 * - Consolidated Dice & Last Roll panel with wireframe polygons/coin.
 *
 * Debugging: removed a stray duplicate fragment after destroyDie() that caused a syntax error.
 * All JSX returns are inside App().
 */
...
// SVG dice/coin icon (wireframe). N=2 -> coin; N>=3 -> regular N-gon.
    ...
    <svg width={s} height={s} viewBox={`0 0 ${s} ${s}`} className="shrink-0">
      ...
      {label != null && (
        <text x="50%" y="52%" textAnchor="middle" dominantBaseline="middle" fontSize={s * 0.42}
          fontFamily="ui-sans-serif, system-ui" fill="currentColor">
          {String(label)}
        </text>
      )}
    </svg>
...
// ---------------- Economics ----------------
// Add-a-Die base cost for K current dice
function costAddDie(k: number) { return 20 * Math.pow(2, Math.max(0, k - 1)); }
function costAddSideFor(sides: number) { return Math.round(10 + 2 * sides); }
function costAddWinningFaceFor(w: number) { return Math.round(20 * Math.pow(1.3, w)); } // gentle scaling
function costAutoRoller(level: number) { return Math.round(50 * Math.pow(1.6, Math.max(0, level))); } // gentle
function costAutoBanker() { return 60; } // fixed; scaled only by global price scale

// Auto-roller interval (ms) for level >=1
...
// Types
interface Die { id: number; sides: number; winningFaces: number; }

// Compute starting heat from dice: +0.1 per side above 2
function computeStartingHeat(dice: Die[]) {
  const units = dice.reduce((acc, d) => acc + Math.max(0, d.sides - 2), 0);
  return 1 + 0.1 * units;
}
// Base payout multiplier per success from dice count
function payoutFromDiceCount(nDice: number) { return 1 + 0.1 * Math.max(0, nDice - 1); }
// Guard: can we destroy a die given current count?
function allowDestroy(nDice: number) { return nDice > 1; }
...
(function runSelfTests() {
  try {
    ...
    const twoD3: Die[] = [{ id: 1, sides: 3, winningFaces: 1 }, { id: 2, sides: 3, winningFaces: 1 }];
    ...
    // New tests: winning face cost increases gently
    ...
    // Auto-roller interval shrinks with level and is >= 10ms
    ...
    // Simple bust probability sanity: single d2 with 1 win face -> 50% bust
    const bustSingleD2 = 1 - (1/2);
    console.assert(Math.abs(bustSingleD2 - 0.5) < 1e-9, "single d2 bust should be 0.5");

    // EXTRA tests
    // interval null for 0/negative levels
    ...
    // mixed dice heat: d3 + d5 => sides above 2 = 1 + 3 = 4 -> heat 1.4
    const mix: Die[] = [{ id: 1, sides: 3, winningFaces: 1 }, { id: 2, sides: 5, winningFaces: 1 }];
...
function App() {
  ...
  const heatRaw = startingHeat + 0.05 * streak; // +0.05 per success
  const heatPay = Math.floor(heatRaw);           // only integer heat multiplies payout

  // Derived roll stats (hoisted for hook-order correctness)
  ...
  // Auto/Banker
  const [autoRollerLevel, setAutoRollerLevel] = useState(0); // 0 = not owned
  ...
  // Price scaling
  const [addDieInflation, setAddDieInflation] = useState(1); // +25% on non-die purchases
  const [globalPriceScale, setGlobalPriceScale] = useState(1); // ×0.5 per destroyed die (cumulative)
  const [destroyedSides, setDestroyedSides] = useState(0);     // for interest rate

  // Two-click confirm for destroy (works even if window.confirm is blocked)
  const [armedDestroyId, setArmedDestroyId] = useState<number | null>(null);
  const armTimer = useRef<number | null>(null);
  useEffect(() => {
    return () => { if (armTimer.current) { clearTimeout(armTimer.current); armTimer.current = null; } };
  }, []);
  ...
  // Passive interest (compounds every 1s)
  useEffect(() => {
  ...
  function appendLog(s: string) { setLog(prev => [s, ...prev.slice(0, 7)]); }
  ...
  function bankNow() {
    if (pot <= 0) { appendLog("Nothing to bank."); return; }
    setBank(b => b + pot);
  ...
  // ---- Upgrades ----
  function buyDie() {
    if (bank < priceDie) { appendLog("Insufficient funds for die."); return; }
  ...
  function bumpAddDiePrice() { setAddDieInflation(x => x * 1.25); }

  function buySide() {
    if (!selected || !canAddSide) return;
    if (bank < priceSide) { appendLog("Insufficient funds for side."); return; }
    setBank(b => b - priceSide);
    setDice(ds => ds.map(d => d.id === selected.id ? { ...d, sides: d.sides + 1, winningFaces: Math.min(d.winningFaces, d.sides) } : d));
    bumpAddDiePrice();
  ...
  function buyWinningFace() {
    ...
    if (bank < priceWin) { appendLog("Insufficient funds for winning face."); return; }
    setBank(b => b - priceWin);
    setDice(ds => ds.map(d => d.id === selected.id ? { ...d, winningFaces: Math.min(d.winningFaces + 1, d.sides - 1) } : d));
    bumpAddDiePrice();
  ...
  function buyOrUpgradeAutoRoller() {
    if (bank < priceAutoRoll) { appendLog("Insufficient funds for Auto Roller."); return; }
  ...
  function toggleAutoRoller() {
    if (autoRollerLevel <= 0) { appendLog("Buy the Auto Roller first."); return; }
    setAutoRollerActive(a => {
      const na = !a;
      appendLog(`Auto Roller ${na ? "started" : "stopped"}.`);
      return na;
    });
  }
  ...
  function buyAutoBanker() {
    ...
    if (bank < priceAutoBanker) { appendLog("Insufficient funds for Auto Banker."); return; }
  ...
  function toggleAutoBanker() {
    if (!autoBankerOwned) { appendLog("Buy the Auto Banker first."); return; }
    setAutoBankerActive(a => {
      const na = !a;
      appendLog(`Auto Banker ${na ? "enabled" : "disabled"}. Target ${autoBankTarget}.`);
      return na;
    });
  }
  ...
  function destroyDie(id: number) {
    setDice(ds => {
      ...
      if (!die) return ds;
      // Apply effects: halve prices and add interest, then remove die
      setGlobalPriceScale(s => Math.max(0.0001, s * 0.5));
      setDestroyedSides(s => s + die.sides);
      const nd = ds.filter(d => d.id !== id);
      if (selectedDieId === id) setSelectedDieId(nd.length ? nd[0].id : -1);
      appendLog(`Destroyed die #${id} (d${die.sides}). Prices halved; interest +${(die.sides*0.1).toFixed(1)}%/tick.`);
      return nd;
    });
  }
